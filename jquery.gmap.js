...
(function($) {
	...
	var DATA_INITIAL_VIEW = "3.459,-76.529"; // aquì obtener las coordenadas del usuario
	...
	var commands = {
		...
		addMarkers: function(data) {
			...
			var lat=window.localStorage.getItem("latitud");//"3.374963638067219";  
			var lon =window.localStorage.getItem("longitud");// "-76.51804110565183";
			if (lat && lon) {
				var myLocation = new google.maps.LatLng(lat, lon);
				new google.maps.Marker({ map:map, position:myLocation, icon:"http://labs.google.com/ridefinder/images/mm_20_green.png", title:"Ud. está aquí!" });
	...
	$.fn[pluginName] = function() {
		...
		if ($.isFunction(cmd)) {
			...
			alert('Función '+arguments[0]+' no soportada por el plugin '+pluginName+' ');
		}
		return this;
	};
})(jQuery);
